...
let signClient=null,qrBox=null,qrLib=null,activeSession=null,activeAddress=null;
...
async function showQR(uri){
 ...
 status('📱 Scan the QR with MetaMask on your phone, then approve.');
}
...
function makeEip1193(client,session,address){
 return {request:async({method,params=[]})=>{
   if(method==='eth_accounts')return [address];
   if(method==='eth_chainId')return '0xaa36a7';
   return client.request({topic:session.topic,chainId:'eip155:11155111',request:{method,params}});
  },on:()=>{},removeListener:()=>{}};
}
async function connectWallet(){
 ...
 try{
  ...
  const {uri,approval}=await client.connect({requiredNamespaces:{eip155:{methods:['eth_sendTransaction','personal_sign','eth_signTypedData','eth_signTypedData_v4','eth_chainId','eth_accounts','eth_getBalance','eth_blockNumber','eth_call','eth_estimateGas','eth_getTransactionByHash','eth_getTransactionReceipt'],chains:['eip155:11155111'],events:['accountsChanged','chainChanged']}}});
  ...
  activeSession=session;activeAddress=address;
  window.walletProvider=makeEip1193(client,session,address);
