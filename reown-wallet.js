...
async function getClient(){
 ...
 const mod=await import('https://esm.sh/@metamask/connect-evm@0.2.0?bundle');
 const createEVMClient=mod.createEVMClient;
 if(!createEVMClient)throw Error('MetaMask Connect EVM failed to load.');
 evmClient=await createEVMClient({dapp:{name:'HyperStream 3D NFT Studio',url:'https://hartensteindominic.github.io/vr-3d-nft-scanner/'}});
 return evmClient;
}
async function connectWallet(){
 ...
 try{
  ...
  status('🦊 Sending connection request to MetaMask…');
  ...
  window.walletClient=client;window.walletSession=result;window.walletProvider=provider;
